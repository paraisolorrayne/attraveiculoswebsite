 ...
 *
 * Mesmo desenho da página de marca: `basePath` mantém navegação, breadcrumb,
 * origem do lead e schema coerentes com a rota que está renderizando. Sem isso,
 * quem entra por /ferrari/roma é jogado para /comprar/* no primeiro clique.
 */
/**
 * Modelo da URL → modelo curado OU derivado do estoque.
 *
 * Exportado porque as duas rotas (/comprar/[brand]/[model] e /[brand]/[model])
 * precisam dele no generateMetadata e na verificação de 404. Era um helper
 * local da rota antiga; ao extrair a página para componente compartilhado ele
 * ficou para trás e quebrou a compilação — nem todo modelo está no catálogo,
 * alguns só existem no estoque.
 */
export async function resolverModelo(brandSlug: string, modelSlug: string) {
...
export async function ModelLandingPage({ brandSlug, modelSlug, basePath }: { brandSlug: string; modelSlug: string; basePath: string }) {
	...
	if (!curado && !derivado) {
		if (findSEOBrand(brandSlug)) redirect(`${basePath}/${brandSlug}`)
		notFound()
	}
	...
	const breadcrumbItems = [
		...
		{ label: brand.displayName, href: `${basePath}/${brandSlug}` },
		{ label: model.name },
	]
					...
					<Container>
						...
						<div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
							{otherModels.map(m => (
								<Link
									key={m.slug}
									href={`${basePath}/${brandSlug}/${m.slug}`}
			...
			<script
				...
				dangerouslySetInnerHTML={{
					__html: JSON.stringify({
						...
						url: `${SITE_URL}${basePath}/${brandSlug}/${modelSlug}`,
						breadcrumb: {
							'@type': 'BreadcrumbList',
							itemListElement: [
								// O nível "Comprar" só existe na rota /comprar — em /ferrari/roma o
								// breadcrumb começa na marca, e um item apontando para /comprar
								// mandaria o rastreador para fora do ramo que ele está lendo.
								...(basePath === '/comprar'
									? [{ '@type': 'ListItem', position: 1, item: { '@id': `${SITE_URL}/comprar`, name: 'Comprar' } }]
									: []),
								{ '@type': 'ListItem', position: 2, item: { '@id': `${SITE_URL}${basePath}/${brandSlug}`, name: brand.displayName } },
								{ '@type': 'ListItem', position: 3, item: { '@id': `${SITE_URL}${basePath}/${brandSlug}/${modelSlug}`, name: model.name } },
							],
						},
